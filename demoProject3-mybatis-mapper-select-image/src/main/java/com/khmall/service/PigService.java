...
public class PigService {
	...
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		...
		//String uploadDir = "C:/Users/user1/Spring-workspace/demoProject3-mybatis-mapper-select-image/src/main/resources/static/images";
		...
		if(!imgFile.exists()	/*만약에 이미지 폴더가 존재하지 않을 때가 true인 ! 추가 */) {
			//폴더 생성하기
			imgFile.mkdirs(); // 폴더가 존재하지 않을 경우 우리가 폴더에 생성
		}
		...
		try {
			...
			// 만약에 파일 경로와 이름을 같이 저장하길 원한다면 아래 코드를 사용
			pig.setPig_image_path("/images/" + fileName); // -> data에 /images/파일명.jpg
			/**************uploadDir -> file Name 변경해서 추가 ***************/
